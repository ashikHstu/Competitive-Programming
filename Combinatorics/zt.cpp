...
bool possible(int k){
    ...
    in_range(i,0,zero_pos.size()-k-2){
        int L=zero_pos[i]+1,R=zero_pos[i+k+1]-1;
        int one_inside_range;
        if(L>R){
            one_inside_range=0;
        }
        else{
        ...
        int total_ones=n-(zero_pos.size()-2);
        int one_removed=(total_ones-one_inside_range);
