...
void workWithNegative(int halfS)
{
    ...
    for (int i = 1; i <= n; i++)
    {
        if (ar[i] == -1 && prevAche == 1 && hoicheVai < halfS)
        {
            vec[vecSize - 1].second++;
            hoicheVai++;
            prevAche = 0;
        }
        else
...
void workWithPositive(int halfS)
{
    vector<pair<int, int>> vec;
    int vecSize = 0;
    int prevAche = 0;
...
void solve()
{
    ...
    if (sum < 0)
    {
        sum *= (-1);
        sum /= 2;
        workWithNegative(sum);
    }
    else
    {
        sum /= 2;
        workWithPositive(sum);
    }
}
