...
void findRes(ll pos,ll curNonZero,ll curValue)
{
    if(pos==18)
    {
        res.pb(curValue);
    ...
    findRes(pos+1,curNonZero,curValue*10); // going forward with next zero digit
    if(curNonZero<3)
    {
        for(ll i=1;i<=9;i++)
        {
            findRes(pos+1,curNonZero+1,curValue*10+i);  // going forward with nonZero
        }
    }
}
...
{
     ll l,r;cin>>l>>r;
     ll result=upper_bound(res.begin(),res.end(),r)-lower_bound(res.begin(),res.end(),l);
     cout<<result<<endl;
}
