...
ll res=0;
      for(ll i=(-1)*n;i<=n;i++)
      {
        if(abs(i)<2 || abs(i)>n)continue;
        //cout<<"cur : "<<i<<", : ";
       
       for(ll j=2;j<=n && abs(i)*j<=n ;j++)
       {
          // cout<<j<<" ";
           ll m1=i*j;
           if(abs(m1)>=2 && abs(m1)<=n && mp[i]!=m1 && mp[m1]!=i)
           {
            
             mp[i]=m1;
             mp[m1]=i;
           //  cout<<"added : "<<abs(j)<<endl;
             res+=(abs(j));
           }
           ...
           if(abs(m2)>=2 &&  abs(m2)<=n && mp[i]!=m2 && mp[m2]!=i)
           {
            mp[i]=m2;
            mp[m2]=i;
            // cout<<"added 2: "<<abs(j)<<endl;
            res+=(abs(j));
           }
