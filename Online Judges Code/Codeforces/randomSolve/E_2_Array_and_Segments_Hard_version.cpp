...
void solve()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        lft[vec[i].second].pb(vec[i].first);
        rgt[vec[i].first].pb(vec[i].second);
    }

    vector<int> ansVec(n, -INF);
    vector<int> add(n + 1, 0);
    int mn = ar[0];

    for (int i = 0; i < n; i++)
    {
        ansVec[i] = max(ansVec[i], ar[i] - mn);
        ...
        for (auto lL : lft[i])
        {
            for (int j = lL; j <= i; j++)
            {
                ...
                --add[j];
                mn = min(mn, ar[j] + add[j]);
            }
        }
        mn = min(mn, ar[i] + add[i]);
    }
    ...
    int ans = *max_element(ansVec.begin(), ansVec.end());
