...
bool pairC(const pair<pair<int,int>,int> &a,
...
bool pairC2(const pair<pair<int,int>,int> &a,
              const pair<pair<int,int>,int> &b)
{
    if(a.first.first==b.first.first)return a.first.second<b.first.second;
    return (a.first.first < b.first.first);
...
void solve()
{
    int n;
      cin>>n;
    vector<pair<pair<int,int>,int>>leftRange,rightRange;
    ...
    sort(leftRange.begin(),leftRange.end(),pairC2);
    sort(rightRange.begin(),rightRange.end(),pairC);

//  cout<<"checking LeftRange : \n";
//  for(int i=0;i<n;i++)cout<<leftRange[i].second<<" : "<<leftRange[i].first.first<<endl;

//  cout<<"checking Right Range : \n";
//  for(int i=0;i<n;i++)cout<<rightRange[i].second<<" : "<<rightRange[i].first.first<<endl;
    
    int needRight,needLeft;
    for(int i=0;i<n;i++)
    {
        if(leftRange[n-1].second!=rightRange[i].second)
        {
            needRight=rightRange[i].first.first;
            break;
        }
    }

    int res=needRight-leftRange[n-2].first.first;

    if(res<0)res=0;
    
     for(int i=n-1;i>=0;i--)
     {
        if(leftRange[i].second!=rightRange[0].second)
        {
            needLeft=leftRange[i].first.first;
            break;
        }
     }

     int res2=rightRange[1].first.first-needLeft;
     //cout<<"res 1 : "<<res<<endl;
