...
TP IL void write(T);
IL bool read(char&);
IL void write(const char*);
TP IL bool read(T&);
IL bool read(char*);
IL void write(char);
HP IL void write(H,T...);
HP IL bool read(H&,T&...);

TP void in(T *a, size_t n)
{
    For(i,0,n) read(a[i]);
}
TP void in(vector<T> &a, size_t n)
{
    For(i,0,n) read(a[i]);
}
TP void out(T *a, size_t n)
{
    For(i,0,n)
    {
        if(i)pc(gp);
...
TP void out(vector<T> &a, size_t n)
{
    For(i,0,n)
    {
        if(i)pc(gp);
...
TP IL T lowestSetBit(T x)
{
    return (x & (-x));
}
TP IL void setBit(T &x, int p)
{
    x |= (1LL<<p);
}
TP IL T highestSetBit(T x)
{
    return (1LL<<(__lg(x)));
}
TP IL bool isSet(T x, int p)
{
    return x & (1<<p);
}
TP IL void toggleBit(T &x, int p)
{
    x ^= (1LL<<p);
}
TP IL void unsetBit(T &x, int p)
{
    x &= (~(1LL<<p));
}
IL bool isPowerOfTwo(ll x)
{
    return x && (!(x & (x-1)));
}
IL int numberOfSetBit(ll x)
{
...
#define TPT template<typename T>
    ...
    TPT IM &operator += (T ot)
    {
        return *this += modint<MOD>(ot);
    }
    TPT IM &operator -= (T ot)
    {
        return *this -= modint<MOD>(ot);
    ...
    TPT IM &operator /= (T ot)
    {
        return *this *= modint<MOD>(ot).inv();
    }
    TPT IM operator + (T ot) const
    {
        return *this + modint<MOD>(ot);
    }
    TPT IM operator - (T ot) const
...
UP string to_string(pair<T, U> p)
{
    return "(" + to_string(p.first) + ", " + to_string(p.second) + ")";
}
TP string to_string(T v)
{
    bool first = true;
    string res = "{";
    for(const auto &x : v)
...
void lca_dfs(int t, int v, int p)
{
    par[t][v][0] = p;
    h[t][v] = h[t][p] + 1;
    for(int i=1; i<20; i++)
        if(par[t][v][i-1] != -1)
            par[t][v][i] = par[t][par[t][v][i-1]][i-1];
    for(int u : g[v]) if(u != p) lca_dfs(t, u, v);
}

int kth_par(int t, int u, int k)
{
    for(int i=0; i<20; ++i)
        if(k & (1<<i))
            u = par[t][u][i];
...
{
    dfs(0, 1, 0);
    int p = max_element(dist[0], dist[0]+n+1)-dist[0];
    dfs(0, p, 0);
    int q = max_element(dist[0], dist[0]+n+1)-dist[0];
    dfs(1, q, 0);
    return ii(p, q);
}
...
IL void write(const char *s)
{
    pf("%s",s);
}
HP IL void write(H h, T ...t)
{
    write(h);
    write(t...);
}
TP IL void write(T x)
{
    if(x==0)
    {
        pc('0');
...
HP IL bool read(H& h, T&... t)
{
    return read(h)&&read(t...);
}
TP IL bool read(T &val)
{
    val=0;
    bool neg=0;
    char ch=gc;
