...
void solve()
{
    ...
    cin >> n;
    int ar[n + 3];
    vector<pair<int, int>> leaves, others, result;
    ...
    int lmt=others.size()-1;
    for(int i=0;i<lmt;i++)
    {
        result.push_back({others[i].second,others[i+1].second});
        others[i].first--;others[i+1].first--;
    }
    ...
    if(others.size()==1)
    {
        if(others[0].first<leaves.size())
        {
            cout<<"NO\n";
            return;
        }
        cout<<"YES ";
         if(leaves.size()>=2)cout<<"2\n";
         else cout<<"1\n";
         cout<<leaves.size()<<endl;
        for(int i=0;i<leaves.size();i++)
        {
           cout<<others[0].second<<" "<<leaves[i].second<<endl;
        }
    ...
    if(others.size()>=1 && leaves.size()>0)
    {
        pair<int, int> pp = leaves.back();
                result.pb({pp.second, others[0].second});
                leaves.pop_back();
                others[0].first--;
                dm++;
    }
    ...
    if(others.size()>1 && leaves.size()>0){
        pair<int, int> pp = leaves.back();
                result.pb({pp.second, others[others.size()-1].second});
                leaves.pop_back();
                others[others.size()-1].first--;
                dm++;
    }
    ...
    for (int i = 0; i < others.size(); i++)
    {
        ...
        while (leaves.size() >= 1 && others[i].first >= 1)
        {
            pair<int, int> pp = leaves.back();
            result.pb({pp.second, others[i].second});
            leaves.pop_back();
            others[i].first--;
    ...
    if(leaves.size()==0)
    {
        ...
        for(pair<int,int>r:result)cout<<r.first<<" "<<r.second<<endl;
    }
    else {
        cout<<"NO\n";
    }
