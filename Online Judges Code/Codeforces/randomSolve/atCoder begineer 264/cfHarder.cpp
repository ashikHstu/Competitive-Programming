...
template<class T> struct pbds_cmp
{
...
bool ok(int mid)
{
    int choto=0,shomanBoro=0,crossKorche=0;
    ...
    for(int i=0; i<n; i++)
    {
        ...
        {
            crossKorche++;
            crossed.pb(vec[i].first);
        }
    }

    int chotoLagbe=max(0ll,(((n+1)/2)-shomanBoro));
    if(chotoLagbe>crossed.size())return false;

    for(int i=0; i<crossed.size(); i++)
    {
        if(i<crossed.size()-chotoLagbe)sum+=crossed[i];
        else sum+=mid;
    }
...
vector<ll> computeMin(vector<ll>vec)
{
    int n=vec.size();

    vector<ll>minA(n+1,LL_INF);
    minA[0]=0;
...
void solve()
{
    ...
    v.pb(2);
    v.pb(1);
    vector<long long>rr=computeMin(v);
    for(int res:rr)cout<<res<<" ";
    cout<<endl;
