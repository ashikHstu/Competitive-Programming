              ...
              const pair<int,int> &b)
{
 ...
 int r1,c1,r2,c2;
 int matrixA[15][15],matrixB[15][15];
 ...
 bool checkCommon()
 {
    // cout<<"checking now : "<<endl;
      int curRow=0;
      vector<int>newMatrix[r2+3];
     for(int i=0;i<r1;i++)
     {
         if(ar_r[i]==0)continue;
         for(int j=0;j<c1;j++)
         {
             if(ar_r[i]==1 && ar_c[j]==1)
             {
                 newMatrix[curRow].pb(matrixA[i][j]);
             }
     ...
     for(int i=0;i<r2;i++)
     {
         for(int j=0;j<c2;j++)
         {
             if(matrixB[i][j]!=newMatrix[i][j])return false;
         }
     }
   ...
   for(int i=0;i<r1;i++)
    {
     ...
     for(int i=0;i<r2;i++)ar_r[i]=1;
     for(int i=r2;i<r1;i++)ar_r[i]=0;

     for(int i=0;i<c2;i++)ar_c[i]=1;
     for(int i=c2;i<c1;i++)ar_c[i]=0;
     sort(ar_r,ar_r+r1);sort(ar_c,ar_c+c1);

     do{

         do{
            bool isOk=checkCommon();
            if(isOk){
                cout<<"Yes\n";
                return;
            }
         }
         while(next_permutation(ar_c, ar_c +c1));

     }
     while(next_permutation(ar_r, ar_r +r1));
