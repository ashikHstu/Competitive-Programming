...
void makeGrid()
{
    ...
    for(int gn=1; gn<=8; gn++)
    {


        for(int i=gn; i<=15-gn+1; i++)
        {
            if(gn%2==1)grid[gn][i]=grid[i][gn]=grid[15-gn+1][i]=grid[i][15-gn+1]=1;
            else {
                grid[gn][i]=grid[i][gn]=grid[15-gn+1][i]=grid[i][15-gn+1]=2;
            }
        }
    }
}
