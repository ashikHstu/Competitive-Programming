...
int findSol(vector<int>&v,int cur)
{
    if(cur<0)return 0;
    ...
    {
        if((v[i]&(1<<cur)))on.pb(v[i]);
        else off.pb(v[i]);
    }

    if(on.size()==0)return findSol(off,cur-1);
    if(off.size()==0)return findSol(on,cur-1);

    return min(findSol(on,cur-1),findSol(off,cur-1))+(1<<cur);
