...
struct point_i     /// Where possible, need to work with integer(point_i)
{
    int x,y;
    point_i()
    {
...
struct point
{
    ...
    bool operator<(point other)const   /// overriding less than operator
    {

        if(fabs(x-other.x)>EPS)  /// use EPS when testing equality of two floatig points
            return x<other.x;
        return y<other.y;
    }
...
int main()
{
   // double v=hypot(3,4);
    //cout<<"v : "<<v<<endl;
    point p1(0,0),p2(0,0),p3(3,3);
    cout<<(p1==p2)<<endl;
    cout<<(p1==p3)<<endl;

    cout <<dist(p1,p3)<<endl;
}
