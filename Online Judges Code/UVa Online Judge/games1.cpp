...
inline void solve(int cs){
    ...
    queue<ii> qc, q; int bird = 0;
    For(i, 0, n){
        For(j, 0, m){
            if(s[i][j] == 't'){
                q.push(ii(i, j));
            }else if(s[i][j] == 'b') ++bird;
    ...
    while(q.size()){
        ...
        For(k, 0, 4){
            int x = tp.first + dx[k];
            int y = tp.second + dy[k];
            if(x>=0 && x<n && y>=0 && y<m && !vis[x][y] && (s[x][y]=='.' || s[x][y]=='b')){
                vis[x][y] = 1;
                bird -= (s[x][y] == 'b');
                q.push(ii(x, y));
            }
