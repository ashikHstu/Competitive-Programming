...
void invalid()
{
    cout << "Invalid Birthday\n";
}
bool is31DaysMonth(const std::string &month)
{
    std::unordered_set<std::string> monthsWith31Days = {
        "January", "March", "May", "July", "August", "October", "December"};
    return monthsWith31Days.find(month) != monthsWith31Days.end();
}
void solve()
{
    int date, year;
    ...
    if (month == "February")
    {
        if (isLeapYear(year) && date == 29)
        {
            if (isLeapYear(year + 4))
                cout << date << " " << month << " " << year + 4 << endl;
            else
                cout << date << " " << month << " " << year + 8 << endl;
        }
        else if (date >= 29)
        {
            invalid();
