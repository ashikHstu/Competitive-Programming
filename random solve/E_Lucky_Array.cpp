...
int isLucky[20000], fTree[20000004], n, m;

void luckyCalculate(int cur)
{
    isLucky[cur] = 1;
    if (cur <= 1000)
    {
        luckyCalculate(cur * 10 + 4);
        luckyCalculate(cur * 10 + 7);
    }
}
...
long long int Query(int ind)
{
    ...
    {
        sum += fTree[ind];
        ind -= (ind & (-ind)); // (ind&(-ind)) will give the rightmost set bit
    }
    return sum;
}
