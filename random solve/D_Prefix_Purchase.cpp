...
void solve()
{
    ...
    for (int i = 0; i < vec.size(); i++)
    {
        ...
        if (i == 0)
        {
            int parbo = k / (vec[i].first);
            k = k % (vec[i].first);
            res.pb({parbo, vec[i].second});
            resAche++;
            last = vec[i].first;
        }
        ...
        else
        {
            ...
            int ex = vec[i].first - last;
            int exp = k / ex;
            exp = min(exp, res[resAche - 1].first);
            k = k - (ex * exp);
            // k = k % ex;
            last = vec[i].first;
    ...
    {
        mainRes[res[i].second] = res[i].first;
    }
    for (int i = n - 1; i >= 0; i--)
    {
        if (mainRes[i] == 0)
            mainRes[i] = mainRes[i + 1];
    }
    for (int i = 0; i < n; i++)
        cout << mainRes[i] << " ";
    cout << endl;
