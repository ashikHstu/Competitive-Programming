...
void solve()
{
    ...
    while (st1.size() <= m * st1.size())
    {
        //   cout << "comparing from every position : " << endl;
        for (int i = 0; i < st1.size() && i + st2.size() <= st1.size(); i++)
        {
            for (int j = 0; j < st2.size(); j++)
            {
                //  cout << st1[i + j] << " == " << st2[j] << endl;
                if (st1[i + j] != st2[j])
                {
                    break;
                }
                else if (j == st2.size() - 1)
                {
                    cout << res << endl;
                    return;
                }
