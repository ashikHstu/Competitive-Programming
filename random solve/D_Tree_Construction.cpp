...
BST *addNode(int v, BST *bst, int par)
{
    // cout << "par , val : " << par << ", " << v << endl;
    if (bst == NULL)
    {
        return newBST(v, par);
    }
    ...
    {
        bst->right = addNode(v, bst->right, bst->val);
    }
    else
    {
        bst->left = addNode(v, bst->left, bst->val);
    }
